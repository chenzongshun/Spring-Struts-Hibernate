...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btnEp").click(function(){
			// 进行表单校验，使用easyUi的校验
			var v = $("#updatePWD").form("validate");
			if(v){// 如果校验成功的话返回true，校验失败easyUi会有css样式提示
				// 判断两次密码是否一致
				var aa = document.getElementById("txtNewPass").value;
				var bb = $("#txtRePass").val();
				if(aa == bb){ // 两次密码一致
					$.post("UserAction_updatePassword",{"password":aa},function(data){
						if(data == '1'){// 修改成功，关闭修改密码的窗口
							$('#editPwdWindow').window('close');
						}else{// 修改失败，弹出提示
							$.message.alert("提示", "密码修改失败！","error");
						}
					})
				}else{ // 错误代表两次密码不一致
					$.messager.alert("提示", "两次密码不一致噢！","warning");
				}
			}
		});
	});
